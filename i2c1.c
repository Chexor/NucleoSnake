...
void InitI2C1(void)
{
	...
	GPIOB->OTYPER |= GPIO_OTYPER_OT_8;																						// Open-drain output (optioneel).
	...
	GPIOB->OTYPER |= GPIO_OTYPER_OT_9;																						// Open-drain output (optioneel).
	
	// HSI als input voor I2C1-module = 8MHz. Dit is de bewezen lab-instelling.
...
// Schrijf ��n byte naar een bepaald I�C-apparaat.
...
// Schrijf array naar een bepaald I�C-apparaat op een bepaald register.
void I2C1WriteRegisterArray(uint8_t deviceAddress, uint8_t registerToWrite, uint8_t* data, uint8_t length)
{
	...
	// LET OP: hier ��n byte bijtellen!
	I2C1->CR2 |= ((length + 1) << 16);									// x bytes te versturen, dus x in NBYTES plaatsen.	
	...
	{
		while((I2C1->ISR & I2C_ISR_TXE) != I2C_ISR_TXE);	// Is verzendbuffer leeg?	
		I2C1->TXDR = data[index];													// Data versturen.
	}
...
// Schrijf array naar een bepaald I�C-apparaat.
void I2C1WriteArray(uint8_t deviceAddress, uint8_t* data, uint8_t length)
{
	...
	// Aantal te verzenden bytes.
	I2C1->CR2 |= (length << 16);												// x bytes te versturen, dus x in NBYTES plaatsen.	
...
// Lees ��n byte van een bepaald I�C-apparaat.
